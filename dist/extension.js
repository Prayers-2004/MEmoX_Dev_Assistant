...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('memox.openChat', () => {
        ...
        chatPanel = vscode.window.createWebviewPanel('memoxChat', 'Memox Chat', vscode.ViewColumn.Two, {
            enableScripts: true,
...
async function scanWorkspace() {
    ...
    for (const folder of workspaceFolders) {
        const files = await vscode.workspace.findFiles('**/*', '**/node_modules/**,**/.git/**,**/dist/**' // Exclude common directories
        );
...
async function checkOllamaStatus() {
    try {
        ...
        const totalMemory = (__webpack_require__(21).totalmem)();
...
async function findSimilarCode(code) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders)
        return undefined;
    const searchResults = await vscode.workspace.findFiles('**/*.{js,ts,py,java,c,cpp}', '**/node_modules/**');
    let best;
...
function getWebviewContent(webview, extensionUri) {
    const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'webview-ui', 'index.js'));
    const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'webview-ui', 'index.css'));
